...
class VisualizationManager:
    ...
    def update_data(self, circle1, circle2, result, coordinates, yaw_data):
        """Update the data to be visualized, including rover position selection"""
        with self.lock:
            self.circle1 = circle1
            self.circle2 = circle2
    
            angle = yaw_data  #accodring to the fix realtion between the start point cs of the rover and global cs, in this case its zero cuz at the start rover cs and global cs is the same
            ...
            rover_x_local = (p1_local[0] - p1_observed_by_rover[0])
            rover_y_local = (p1_local[1] - p1_observed_by_rover[1])

            rover_global = self.convert_global_to_local(rover_x_local, rover_y_local, -1 * angle)
            self.rover_position = tuple(rover_global)
            self.rover_path.append(rover_global)
...
class CircleNode(Node):
    def __init__(self, visualizer=None):
        ...
        self.yaw_subscription = self.create_subscription(Float32,
            '/yaw',
            self.yaw_callback,
            10
        )
    ...
    def listener_callback(self, msg):
        ...
        if len(coords) >=3 :
            ...
            C2 = Circle(points_dict["63"][0], points_dict["63"][1], 6.0)  # for now passing a static C2 variable so the code doesnt break
            result = C1.circle_intersect(C2)
           
            
            # Update visualization if available
            if self.visualizer:
                self.visualizer.update_data(C1, C2, result,coords,yaw_data = self.yaw)
...
def main(args=None):
    ...
    image_path = '/home/atharv/dev_ws/src/ur5_control/ur5_control/mars_yard_top.png'
